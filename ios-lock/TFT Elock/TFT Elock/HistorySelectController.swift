...
class HistorySelectController: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        let btnDateRangeFrame = CGRect.init(x: appFrame.size.width/2 - 100, y: 0, width: 200, height: 30);
...
//        self.view.addSubview(self.datePicker)
    ...
    @objc func submit(){
        var readStartDate = Int64(self.dateStart.timeIntervalSince1970)
        var readEndDate = Int64(self.dateEnd.timeIntervalSince1970)
        if readStartDate > readEndDate {
            Toast.hudBuilder.title(NSLocalizedString("invalid_date", comment: "Invalid date")).show()
            return
        }
        if readEndDate - readStartDate > 259200{
            Toast.hudBuilder.title(NSLocalizedString("date_time_range_out_of_3_day", comment: "The time range of the data cannot exceed 3 days")).show()
            return
        }
