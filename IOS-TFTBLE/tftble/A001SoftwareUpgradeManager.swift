...
class A001SoftwareUpgradeManager  {
    ...
    public var cmdWriteNotifyCharacteristic: CBCharacteristic?
    public var dataWriteCharacteristic: CBCharacteristic?
    public static let upgradeDataServiceId = CBUUID(string: "27770001-999c-4d6a-9fc4-c7272be10900")
    public static let dataWriteUUID = CBUUID(string: "27770003-999c-4d6a-9fc4-c7272be10900")
    public static let cmdWriteNotifyUUID = CBUUID(string: "27770002-999c-4d6a-9fc4-c7272be10900")
...
//                    var curPackageOffset = curPackage * onePackLen
//                    if upgradeFileContentList.count > cur4KPackage,
//                       upgradeFileContentList[cur4KPackage].count == curPackage {
//                        let allLen = upgradeFileContentList[cur4KPackage].count
//                        curPackageOffset = allLen
//                    }
//                    let offset = Utils.unSignedInt2Bytes(Int64(cur4KPackage * 4096 + curPackageOffset))
...
//        TftBleConnectManager.getInstance().writeUpgradePackageDataArray(content: content)
        print("upgrade package write:" + (Utils.bytes2HexString(bytes: content, index: 0) ?? ""))
        let data = Data(bytes: content, count: content.count)
        self.cbPeripheral.writeValue(data, for: self.dataWriteCharacteristic!, type: .withoutResponse)
    }

    private func writeArray(_ content: [UInt8]) {
        lastWriteDate = Date()
//        TftBleConnectManager.getInstance().writeUpgradeCmdDataArray(content: content)
        print("upgrade package cmd:" + (Utils.bytes2HexString(bytes: content, index: 0) ?? ""))
        let data = Data(bytes: content, count: content.count)
        self.cbPeripheral.writeValue(data, for: self.cmdWriteNotifyCharacteristic!, type: .withResponse)
    }
