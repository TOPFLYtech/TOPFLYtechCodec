...
import UIKit
import CLXToast
class EditAccessPwdController : UIViewController{
    ...
    override func viewDidLoad() {
        ...
        titleLabel.text = NSLocalizedString("ble_pwd", comment: "ble_pwd")
...
//        self.submitBtn.ghostColor = UIColor.colorPrimary
    ...
    var delegate : EditAccessPwdDelegate?
    @objc private func submit(){
        ...
        if oldPwd != self.confirmPwd{
            Toast.hudBuilder.title(NSLocalizedString("pwd_not_match", comment: "Old password is wrong, please re-enter!")).show()
            return
        }
        if newPwd != repeatPwd{
