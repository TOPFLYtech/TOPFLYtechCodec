...
public class ObdDecoder {
    ...
    private static final byte[] SERCOND_SIGNUP =      {0x26, 0x26, 0x31};
...
//            bleTireData.setIsTireLeaks(isTireLeaks);
    ...
    private SignInMessage parseSecondLoginMessage(byte[] bytes) throws ParseException {
        ...
        int obdV1 = (int)bytes[27];
        int obdV2 = (int)bytes[28];
        int obdV3 = (int)bytes[29];
        if(obdV1 < 0){
            obdV1 += 256;
        }
        if(obdV2 < 0){
        ...
        String obdHardware = String.format("%d.%d", (bytes[36] & 0xf0) >> 4, bytes[36] & 0xf);
