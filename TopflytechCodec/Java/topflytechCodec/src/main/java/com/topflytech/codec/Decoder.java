...
public class Decoder {
    ...
    private static final byte[] SERCOND_SIGNUP =      {0x25, 0x25, 0x31};
    ...
    private static final byte[] MANUAL_CAN_DATA = {0x25, 0x25, (byte)0x44};

    private static final byte[] INDEFINITE_LOCATION_DATA =  {0x25, 0x25, (byte)0x62};
    private static final byte[] INDEFINITE_LOCATION_ALARM_DATA =  {0x25, 0x25, (byte)0x64};
    ...
    private static boolean match(byte[] bytes) {
        ...
        return Arrays.equals(SIGNUP, bytes)
                ...
                || Arrays.equals(MANUAL_CAN_DATA,bytes)
...
//            System.out.println("Error Message,Topflytech Codec Msg Length Error");
    ...
    private ManualCANMessage parseManualCANMessage(byte[] bytes) {
        ...
        manualCANMessage.setSerialNo(serialNo);
        manualCANMessage.setImei(imei);
        manualCANMessage.setOrignBytes(bytes);
        return manualCANMessage;
    }
    ...
    public void build(byte[] bytes,Callback callback) throws ParseException {
        if (bytes != null && bytes.length > HEADER_LENGTH
            ...
            switch (bytes[2]) {
                ...
                case 0x44:
                    ManualCANMessage manualCANMessage = parseManualCANMessage(bytes);
                    callback.receiveManualCANMessage(manualCANMessage);
                case 0x62:
                case 0x64:
                    Message indefiniteLocationMessage = DecoderHelper.parseLocationMessage(bytes);
                    indefiniteLocationMessage.setOrignBytes(bytes);
...
//        boolean isNeedResp = (serialNo & 0x8000) != 0x8000;
        ...
        String hardware = BytesUtils.bytes2HexString(Arrays.copyOfRange(bytes, 24, 25), 0);
...
//        message.setIsNeedResp(isNeedResp);
    ...
    public interface Callback{
        ...
        void receiveManualCANMessage(ManualCANMessage manualCANMessage);
