...
//        }
        ...
        if(!locationMessage.isLatlngValid() && locationMessage.getLatitude() != null && locationMessage.getLongitude() != null){
            if(!BytesUtils.checkLatitudeAndLongtitudeIsZero(locationMessage.getLatitude(),locationMessage.getLatitude())){
    ...
    private static boolean isAllFF(byte[] array) {
        ...
        for (Integer dataId:
            ...
            if(dataId == 0x01){
                ...
                if(isAllFF(valueByte)){
                    locationMessage.setLatlngValid(false);
                    continue;
                }
                ...
                locationMessage.setObdElectricData(false);
                ...
                locationMessage.setObdElectricData(true);
                ...
                locationMessage.setRemainPowerDistance(remainPowerMinDistance);
                ...
                Long remainPowerDistance = BytesUtils.unsigned4BytesToInt(valueByte,4);
                if(remainPowerDistance == 4294967295l){
                    remainPowerDistance = -999l;
                }
                long carChargeVoltageTemp = BytesUtils.unsigned4BytesToInt(valueByte, 8);
                Float carChargeVoltage = carChargeVoltageTemp / 1000.0f;// V
                if(carChargeVoltageTemp == 4294967295l){
                    carChargeVoltage = -999f;
                }

                Long carChargeElectricCurrent = BytesUtils.unsigned4BytesToInt(valueByte,12);//mA
                if(carChargeElectricCurrent == 4294967295l){
                    carChargeElectricCurrent = -999l;
                }
                Long carBatteryEffectiveCapacity = BytesUtils.unsigned4BytesToInt(valueByte,16);//Wh
                if(carBatteryEffectiveCapacity == 4294967295l){
                    carBatteryEffectiveCapacity = -999l;
                }
                Long carBatteryInitialCapacity = BytesUtils.unsigned4BytesToInt(valueByte,20);//Wh
                if(carBatteryInitialCapacity == 4294967295l){
                    carBatteryInitialCapacity = -999l;
                }
                Long carTotalPowerConsumption = BytesUtils.unsigned4BytesToInt(valueByte,24);//Wh
                if(carTotalPowerConsumption == 4294967295l){
                    carTotalPowerConsumption = -999l;
                }
                locationMessage.setObdElectricData(true);
                ...
                locationMessage.setDashboardSpeed(dashboardSpeed);
                locationMessage.setAcceleratorPedalPosition(acceleratorPedalPosition);
                locationMessage.setRemainPowerDistance(remainPowerDistance);
                locationMessage.setCarChargeVoltage(carChargeVoltage);
                locationMessage.setCarChargeElectricCurrent(carChargeElectricCurrent);
                locationMessage.setCarBatteryEffectiveCapacity(carBatteryEffectiveCapacity);
                locationMessage.setCarBatteryInitialCapacity(carBatteryInitialCapacity);
    ...
    private static void parseLocationTypeTwoData(LocationMessage locationMessage, HashMap<Integer, byte[]> typeMap) {
        for (Integer dataId:
            ...
            if(dataId == 0x01){
                ...
                boolean latlngValid =  (value & 0x04) == 0x04;
                locationMessage.setLatlngValid(latlngValid);
                boolean isHistory = (value & 0x02) == 0x02;
                locationMessage.setIsHistoryData(isHistory);
            }else if(dataId == 0x03){
                byte[] valueByte = typeMap.get(dataId);
                int value = valueByte[0];
                ...
                byte[] valueByte = typeMap.get(dataId);
                boolean flashLightOpen = (valueByte[0] & 0x80) == 0x80;
                boolean logoLightOpen = (valueByte[0] & 0x40) == 0x40;
                boolean buzzerOpen = (valueByte[0] & 0x20) == 0x20;
                boolean lostModeOpen = (valueByte[0] & 0x10) == 0x10;
                locationMessage.setFlashLightOpen(flashLightOpen);
                locationMessage.setLogoLightOpen(logoLightOpen);
                locationMessage.setBuzzerOpen(buzzerOpen);
                locationMessage.setLostModeOpen(lostModeOpen);
            }else if(dataId == 0x0A){
                byte[] valueByte = typeMap.get(dataId);
                float deviceTemp = -999;
                if ((valueByte[0] & 0xff) != 0xff){
                ...
                boolean isWirelessCharging = (valueByte[0] & 0x02) == 0x02;
                ...
                locationMessage.setWirelessCharging(isWirelessCharging);
                ...
                locationMessage.setSolarVoltage(solarVoltage/ 1000.0f);
            }else if(dataId == 0x64){
                byte[] valueByte = typeMap.get(dataId);
                boolean isInvalid = BytesUtils.isByteArrayValueInvalid(valueByte,0,valueByte.length);
                if(!isInvalid){
                    boolean isNegative = (valueByte[1] & 0x80) == 0x80;
                ...
                if(!isInvalid){
                    boolean isNegative = (valueByte[1] & 0x80) == 0x80;
                    float value = (isNegative ? -1 : 1) * ((valueByte[0]) + ((valueByte[1] & 0x7f) /100.0f));
                    locationMessage.setDeviceHighPrecisionTemp(value);
                }
              
            }else if(dataId == 0x67){
                byte[] valueByte = typeMap.get(dataId);
                int voltage = BytesUtils.bytes2Short(valueByte,0);
                locationMessage.setAnalogInput1(voltage/ 1000.0f);
            }else if(dataId == 0x68){
                byte[] valueByte = typeMap.get(dataId);
                int voltage = BytesUtils.bytes2Short(valueByte,0);
                locationMessage.setAnalogInput2(voltage/ 1000.0f);
            }else if(dataId == 0x69){
                byte[] valueByte = typeMap.get(dataId);
                int voltage = BytesUtils.bytes2Short(valueByte,0);
                locationMessage.setAnalogInput3(voltage/ 1000.0f);
            }else if(dataId == 0xA0){
                byte[] valueByte = typeMap.get(dataId);
    ...
    private static void parseLocationTypeOneData(LocationMessage locationMessage, HashMap<Integer, byte[]> typeMap) {
        for (Integer dataId:
            ...
            if(dataId == 0x03){
                ...
                byte[] valueByte = typeMap.get(dataId); 
                locationMessage.setAntitheftedStatus(valueByte[0] & 0xff);
            }else if(dataId == 0x80){
                byte[] valueByte = typeMap.get(dataId);
